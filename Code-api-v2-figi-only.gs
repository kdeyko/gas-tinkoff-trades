...
function _getAllInstruments() {
  // 1 - base
  // 2 - all
  // https://tinkoff.github.io/investAPI/instruments/#instrumentstatus
  const default_instruments_amount = 1

  const bonds = _getAllBonds(default_instruments_amount).instruments
  const etfs = _getAllEtfs(default_instruments_amount).instruments
  const shares = _getAllShares(default_instruments_amount).instruments
...
// Функция посылает "тяжелые" запросы, рекомендуется НЕ использовать ее
// Узнать FIGI по тикеру можно, например, тут:
// Telegram:	https://t.me/TIPortfolioBot
function getFigiByTicker(ticker) {
...
function getAccountBalanceByCurrency(accountId, currency, dummy) {
  ...
  if (money.find(x => x.currency === currency) === undefined) {
    return 0
  }
...
function getAccountBalanceByFigi(accountId, figi, dummy) {
  const positions = _getPositions(accountId)
  if (Object.keys(positions).length === 0) {
    return 0
  }
  
  const securities = positions.securities
  if (!securities) {
    return 0
  }
  
  const security = securities.find(x => x.figi === figi)
  if (!security) {
    return 0
  }
...
function getCurrencyByFigi(figi, dummy) {
  const result = _getInstrumentByFigi(figi).currency
  return currencies[result]
}

function getNameByFigi(figi, dummy) {
  return _getInstrumentByFigi(figi).name
}

function getLotsByFigi(figi, dummy) {
  return _getInstrumentByFigi(figi).lot
}

function getPriceByFigi(figi, dummy) {
  const figiList = _getMyFigiList()
  let lastPrices
  if (figiList.includes(figi)) {
    lastPrices = _getPricesByFigiList(figiList)
  } else {
    lastPrices = _apiRequest([figi + '_price', EXPIRATION_DEFAULT], 'MarketDataGetLastPrices', [[figi]]).lastPrices
  }
  ...
  Logger.log(figi + ' price : ' + resultPrice)
  return resultPrice
}

/* ####################################################################################################### */
// Can be called via button
